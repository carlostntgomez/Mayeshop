document.addEventListener("DOMContentLoaded", (event) => {
    ...
    if (document.querySelector(".ul-header-nav")) {
        ...
        ulHeaderNavMobileItems.forEach((item) => { // item is div.has-sub-menu
            const trigger = item.querySelector('a');
            if (window.innerWidth < 992 && trigger) {
                trigger.addEventListener("click", (event) => {
                    // If the click was on an element within a level 2 submenu, do nothing.
                    if (event.target.closest('.has-submenu-level2')) {
                        return;
                    }
        ...
        const ulHeaderNavMobileLevel2Items = ulHeaderNavMobile.querySelectorAll(".has-submenu-level2 > a");
        ulHeaderNavMobileLevel2Items.forEach((item) => { // item is a
            if (window.innerWidth < 992) {
